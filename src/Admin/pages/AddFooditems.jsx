...
import { AddFoodItemsAPI, AddPopularDishesAPI, CategoriesAPI, DeleteFoodItemsAPI, getAFoodItemsAPI, getallCategoriesAPI, getallFoodItemsAPI, getallPopDishesAPI, UpdateFoodItemsAPI } from '../../service/allApi';
...
function AddFooditems() {
  ...
  const [modal2Status, setmodal2Status] = useState(false)
      //modal1 for Edit food items
  const [editModal, seteditModal] = useState(false)
      //modal1 for Edit PopDishes && category 
  const [edit2Modal, setedit2Modal] = useState(false)
  //state foe search
  const [Search,setSearch]=useState("")
    //state for store Edited Food items
  const [EditFoodItems, setEditFoodItems] = useState([])
      //state for store Edited Popular dishes 
  const [EditPopDishes, setEditPopDishes] = useState([])
        //state for store Edited Popular dishes 
  ...
  //edit a Food Items fuction
  const EditFood =async(id)=>{
    try {
      const result=await getAFoodItemsAPI(id)
       if (result.status > 199 || result.status < 300) {
        console.log(result);
        setEditFoodItems(result.data)

      }
      else {
        alert("Something Went worng get a food items")
      }
   ...
   //Update a Food Items fuction
  const UpadteFood =async()=>{
    try {
      const result=await UpdateFoodItemsAPI(EditFoodItems.id,EditFoodItems)
       if (result.status > 199 || result.status < 300) {
        console.log(result);
        alert("Updated Food Items Succefully")
        getallFoods()
        ...
        alert("Something Went worng Update a food item")
      }
    ...
    //Delete a Food Items fuction
  const DeleteFood =async(id)=>{
    try {
      const result=await DeleteFoodItemsAPI(id)
       if (result.status > 199 || result.status < 300) {
        console.log(result);
        alert("Deleted Food Items Succefully")
        getallFoods()  

      }
      else {
        alert("Something Went worng Delete a food item")
      }
                    ...
                    <tbody>
                      {gettAllPopDishes&&gettAllPopDishes.map((dishe)=>(<tr className='border-b border-gray-400 hover:bg-gray-100'>
                        ...
                        <td className='h-10 text-center align-middle p-2 '>
                          <button onClick={()=>setedit2Modal(!edit2Modal)} className='p-2 rounded-xl text-orange-500 border border-orange-500 hover:bg-orange-500 hover:text-white ms-1 me-3'><MdOutlineModeEdit className='text-1xl' /></button>
                          <button className='p-2 rounded-xl bg-red-500 text-white'><RiDeleteBinLine className='text-1xl' /></button>
                        </td>
                      </tr>))}
                    </tbody>
                ...
                <tbody>
                 {gettAllFooditem && gettAllFooditem.filter(foods=> foods?.FName?.toLowerCase().includes(Search.toLowerCase())).map((items)=>(<tr className='border-b border-gray-400 hover:bg-gray-100'>
                    ...
                    <td className='h-20 text-center align-middle '>
                      <button onClick={()=>{seteditModal(!editModal),EditFood(items?.id)}} className='p-2 rounded-xl text-orange-500 border border-orange-500 hover:bg-orange-500 hover:text-white ms-1 me-3'><MdOutlineModeEdit className='text-xl' /></button>
                      <button onClick={()=>DeleteFood(items?.id)}  className='p-2 rounded-xl bg-red-500 text-white'><RiDeleteBinLine className='text-xl' /></button>
                    </td>
                  </tr>))
                  }
                </tbody>
            ...
            <div className='flex justify-center items-center min-h-screen py-8'>
              <div className='bg-white rounded-2xl md:w-160 w-100 p-5  '>
                <div className='bg-white text-black font-bold text-2xl rounded-xl flex justify-between items-center p-5 '>
                  ...
                  <IoCloseSharp onClick={() => seteditModal(!editModal)} className='text-2xl cursor-pointer' />

                </div>
                <div className='flex flex-col justify-center  p-3'>
                  <label className='font-bold text-gray-600' htmlFor="">Name</label>
                  <input value={EditFoodItems.FName} onChange={(e) => setEditFoodItems({ ...EditFoodItems, FName: e.target.value })} className='h-12 rounded-xl border border-gray-300 p-5' type="text" placeholder='e.g : Normal Pizza' />
                </div>
                <div className='flex  justify-between  p-3  gap-2'>
                  <div className='flex flex-col w-full'>
                    <label className='font-bold text-gray-600' htmlFor="">Price ($)</label>
                    <input value={EditFoodItems.price} onChange={(e) => setEditFoodItems({ ...EditFoodItems, price: e.target.value })} className='h-12 rounded-xl border border-gray-300 p-5' type="text" placeholder='10.10' />
                  </div>
                  <div className='flex flex-col w-full'>
                    <label className='font-bold text-gray-600' htmlFor="">Categories</label>
                    <select value={EditFoodItems.category} onChange={(e) => setEditFoodItems({ ...EditFoodItems, category: e.target.value })} className='h-12 rounded-xl border border-gray-300 text-gray-500 font-bold p-3' name="" id=""> 
                        <option  selected >Selecte Category</option>
                       {gettAllcategories&& gettAllcategories.map((categoray)=>(<>
                        <option key={categoray.id} value={categoray.id}>{categoray.icon} {categoray.Name}</option>
                        </>)) }
                ...
                <div className='flex flex-col justify-center  p-3'>
                  <label className='font-bold text-gray-600' htmlFor="">Image URL</label>
                  <input value={EditFoodItems.image} onChange={(e) => setEditFoodItems({ ...EditFoodItems, image: e.target.value })} className='h-12 rounded-xl border border-gray-300 p-5' type="text" placeholder='http//....' />
                </div>
                <div className='flex flex-col justify-center  p-3'>
                  <label className='font-bold text-gray-600' htmlFor="">Preparation Time</label>
                  <input value={EditFoodItems.preparationTime} onChange={(e) => setEditFoodItems({ ...EditFoodItems, preparationTime: e.target.value })} className='h-12 rounded-xl border border-gray-300 p-5' type="text" placeholder='20-25 m' />
                </div>
                <div className='flex flex-col justify-center  p-3'>
                  <label className='font-bold text-gray-600' htmlFor="">Description</label>
                  <textarea value={EditFoodItems.description} onChange={(e) => setEditFoodItems({ ...EditFoodItems, description: e.target.value })} className='rounded-xl border border-gray-300 p-5' name="" id="" cols="3" rows="3" placeholder='Describe dish ... '></textarea>
                </div>
                <div className='flex justify-end p-3 gap-2'>
                  <button onClick={() => seteditModal(!editModal)} className='border border-orange-600 text-orange-600 hover:bg-orange-600 hover:text-white p-3 text-lg rounded-2xl'>Cancel</button>
                  <button onClick={UpadteFood} className='bg-orange-600 text-white p-3 text-lg rounded-2xl'>Update Items</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className='relative z-10 overflow-y-auto '>
          <div className='fixed inset-0 bg-gray-500/75 overflow-y-auto '>
            <div className='flex justify-center items-center min-h-screen py-8'>
              <div className='bg-white rounded-2xl md:w-160 w-100 p-5  '>
                <div className='bg-white text-black font-bold text-2xl rounded-xl flex justify-between items-center p-5 '>
                  <h3>Edit Categories List && Popular Dishes</h3>
                  <IoCloseSharp onClick={() => setedit2Modal(!edit2Modal)} className='text-2xl cursor-pointer' />
                ...
                <div className='flex flex-col gap-3'>
                  <div className='  p-3 '>
                    ...
                    <div className='flex justify-end mt-3 p-2'>
                        <button onClick={Addcategary} className='bg-orange-600 text-white p-3 text-lg rounded-2xl'>Update Cate</button>
                    </div>
                ...
                <div className='flex justify-end p-3 gap-2'>
                  <button onClick={() => setedit2Modal(!edit2Modal)} className='border border-orange-600 text-orange-600 hover:bg-orange-600 hover:text-white p-3 text-lg rounded-2xl'>Cancel</button>
                  <button onClick={AddPopDishes}  className='bg-orange-600 text-white p-3 text-lg rounded-2xl'>Update Dishes</button>
